...
unsafe extern "C" {
    ...
    pub fn create_directory_at(handle: i32, path_ptr: *const u8, path_len: usize, result_ptr: *mut u8);
    ...
    pub fn remove_directory_at(handle: i32, path_ptr: *const u8, path_len: usize, result_ptr: *mut u8);
    #[link_name = "[method]descriptor.rename-at"]
    pub fn rename_at(handle: i32, old_path_ptr: *const u8, old_path_len: usize, new_handle: i32, new_path_ptr: *const u8, new_path_len: usize, result_ptr: *mut u8);
}
...
pub unsafe fn rename(from: &str, to: &str) -> i32 {
    let mut result_buf = [0u8; 4];
    rename_at(3, from.as_ptr(), from.len(), 3, to.as_ptr(), to.len(), result_buf.as_mut_ptr());
    if result_buf[0] == 0 { 0 } else { -1 }
}
