...
use crate::wasm::{
    core::indices::{FuncIdx, TypeIdx},
    core::reader::section_header::{SectionHeader, SectionTy},
    core::reader::span::Span,
    core::reader::types::data::DataSegment,
    core::reader::types::element::ElemType,
    core::reader::types::export::Export,
    core::reader::types::global::{Global, GlobalType},
    core::reader::types::import::{Import, ImportDesc},
    core::reader::types::{FuncType, MemType, ResultType, TableType},
...
fn validate_exports(validation_info: &ValidationInfo) -> Result<(), ValidationError> {
    ...
    use crate::wasm::core::reader::types::export::ExportDesc::*;
...
pub fn validate(wasm: &[u8]) -> Result<ValidationInfo<'_>, ValidationError> {
    ...
    let version_bytes = wasm.strip_bytes::<4>()?;
    if version_bytes == [0x01, 0x00, 0x00, 0x00] {
        debug!("Header ok - Core Module");
    } else if version_bytes == [0x0d, 0x00, 0x01, 0x00] {
        debug!("Header ok - Component");
        let parsed_component = crate::wasm::component::reader::parse_component(&mut wasm)
    ...
    while (skip_section(&mut wasm, &mut header)?).is_some() {}
    let imported_global_types = imports
        .iter()
        .filter_map(|m| match m.desc {
