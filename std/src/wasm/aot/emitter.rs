...
impl X64Emitter {
    ...
    pub fn emit_u64(&mut self, v: u64) { self.code.extend_from_slice(&v.to_le_bytes()); }
    ...
    pub fn rex(&mut self, w: bool, r: u8, x: u8, b: u8) {
        ...
        if x >= 8 { prefix |= 0x02; }
        if b >= 8 { prefix |= 0x01; }
        if prefix != 0x40 || w { self.emit_u8(prefix); }
    }

    pub fn movq_xmm_reg(&mut self, dst: XmmReg, src: Reg) {
        self.emit_u8(0x66); self.rex(true, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0x6E); self.modrm(3, dst as u8, src as u8);
    }

    pub fn shr_reg32_imm32(&mut self, reg: Reg, imm: u32) {
        self.rex(false, 0, 0, reg as u8);
        self.emit_u8(0xC1); self.modrm(3, 5, reg as u8);
    ...
    pub fn jmp_label(&mut self, label_id: usize) {
        ...
        self.emit_u32(0);
        self.relocs.push(Reloc { pos, label_id, kind: RelocKind::Jmp32 });
    }
    ...
    pub fn jcc_label(&mut self, cond: u8, label_id: usize) {
        ...
        let pos = self.code.len();
        self.emit_u32(0);
        self.relocs.push(Reloc { pos, label_id, kind: RelocKind::Jcc32(cond) });
    }
    ...
    pub fn push_reg(&mut self, reg: Reg) {
        if (reg as u8) >= 8 { self.emit_u8(0x41); }
    ...
    pub fn addss_xmm_xmm(&mut self, dst: XmmReg, src: XmmReg) {
        ...
        self.emit_u8(0x0F); self.emit_u8(0x58); self.modrm(3, dst as u8, src as u8);
    }
    pub fn subss_xmm_xmm(&mut self, dst: XmmReg, src: XmmReg) {
        self.emit_u8(0xF3); self.rex(false, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0x5C); self.modrm(3, dst as u8, src as u8);
    }
    pub fn mulss_xmm_xmm(&mut self, dst: XmmReg, src: XmmReg) {
        self.emit_u8(0xF3); self.rex(false, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0x59); self.modrm(3, dst as u8, src as u8);
    }
    pub fn divss_xmm_xmm(&mut self, dst: XmmReg, src: XmmReg) {
        self.emit_u8(0xF3); self.rex(false, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0x5E); self.modrm(3, dst as u8, src as u8);
    }
    pub fn sqrtss_xmm_xmm(&mut self, dst: XmmReg, src: XmmReg) {
        self.emit_u8(0xF3); self.rex(false, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0x51); self.modrm(3, dst as u8, src as u8);
    }
    ...
    pub fn ucomiss_xmm_xmm(&mut self, dst: XmmReg, src: XmmReg) {
        self.rex(false, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0x2E); self.modrm(3, dst as u8, src as u8);
    }
    ...
    pub fn paddb_xmm_xmm(&mut self, dst: XmmReg, src: XmmReg) {
        self.emit_u8(0x66); self.rex(false, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0xFC); self.modrm(3, dst as u8, src as u8);
    }

    pub fn pcmpeqd_xmm_xmm(&mut self, dst: XmmReg, src: XmmReg) {
        self.emit_u8(0x66); self.rex(false, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0x76); self.modrm(3, dst as u8, src as u8);
    }

    pub fn pandn_xmm_xmm(&mut self, dst: XmmReg, src: XmmReg) {
        self.emit_u8(0x66); self.rex(false, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0xDF); self.modrm(3, dst as u8, src as u8);
    }

    pub fn pextrd_reg_xmm_imm8(&mut self, dst: Reg, src: XmmReg, imm: u8) {
        self.emit_u8(0x66); self.rex(true, src as u8, 0, dst as u8);
        self.emit_u8(0x0F); self.emit_u8(0x3A); self.emit_u8(0x16);
        self.modrm(3, src as u8, dst as u8);
        self.emit_u8(imm);
    }
    ...
    pub fn pinsrd_xmm_reg_imm8(&mut self, dst: XmmReg, src: Reg, imm: u8) {
        self.emit_u8(0x66); self.rex(true, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0x3A); self.emit_u8(0x22);
        self.modrm(3, dst as u8, src as u8);
        self.emit_u8(imm);
    }
    ...
    pub fn movd_reg_xmm(&mut self, dst: Reg, src: XmmReg) {
        self.emit_u8(0x66); self.rex(true, src as u8, 0, dst as u8);
        self.emit_u8(0x0F); self.emit_u8(0x7E); self.modrm(3, src as u8, dst as u8);
    }
    ...
    pub fn cvtsi2ss_xmm_reg(&mut self, dst: XmmReg, src: Reg) {
        ...
        self.emit_u8(0x0F); self.emit_u8(0x2A); self.modrm(3, dst as u8, src as u8);
    }
    ...
    pub fn movsx_reg_reg8(&mut self, dst: Reg, src: Reg) {
        self.rex(true, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0xBE); self.modrm(3, dst as u8, src as u8);
    }

    pub fn movsx_reg_reg16(&mut self, dst: Reg, src: Reg) {
        self.rex(true, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0xBF); self.modrm(3, dst as u8, src as u8);
    }

    pub fn movzx_reg_reg8(&mut self, dst: Reg, src: Reg) {
        self.rex(true, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0xB6); self.modrm(3, dst as u8, src as u8);
    }

    pub fn movzx_reg_reg16(&mut self, dst: Reg, src: Reg) {
        self.rex(true, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0xB7); self.modrm(3, dst as u8, src as u8);
    }

    pub fn movzx_reg_mem8(&mut self, dst: Reg, base: Reg, offset: i32) {
        self.rex(true, dst as u8, 0, base as u8);
        self.emit_u8(0x0F); self.emit_u8(0xB6); self.emit_modrm_mem(dst, base, offset);
    }
    pub fn movsx_reg_mem8(&mut self, dst: Reg, base: Reg, offset: i32) {
        self.rex(true, dst as u8, 0, base as u8);
        self.emit_u8(0x0F); self.emit_u8(0xBE); self.emit_modrm_mem(dst, base, offset);
    }
    pub fn movzx_reg_mem16(&mut self, dst: Reg, base: Reg, offset: i32) {
        self.rex(true, dst as u8, 0, base as u8);
        self.emit_u8(0x0F); self.emit_u8(0xB7); self.emit_modrm_mem(dst, base, offset);
    }
    pub fn movsx_reg_mem16(&mut self, dst: Reg, base: Reg, offset: i32) {
        self.rex(true, dst as u8, 0, base as u8);
        self.emit_u8(0x0F); self.emit_u8(0xBF); self.emit_modrm_mem(dst, base, offset);
    }
    ...
    pub fn xadd_mem_reg(&mut self, base: Reg, offset: i32, src: Reg, is_64: bool) {
        ...
        self.emit_u8(0x0F); self.emit_u8(0xC1);
        self.emit_modrm_mem(src, base, offset);
    }
    ...
    pub fn xadd_mem_reg8(&mut self, base: Reg, offset: i32, src: Reg) {
        self.lock_prefix();
        self.rex(false, src as u8, 0, base as u8);
        self.emit_u8(0x0F); self.emit_u8(0xC0);
        self.emit_modrm_mem(src, base, offset);
    }
    ...
    pub fn cmpxchg_mem_reg(&mut self, base: Reg, offset: i32, src: Reg, is_64: bool) {
        self.lock_prefix();
        self.rex(is_64, src as u8, 0, base as u8);
        self.emit_u8(0x0F); self.emit_u8(0xB1);
        self.emit_modrm_mem(src, base, offset);
    }
    ...
    pub fn cmpxchg_mem_reg8(&mut self, base: Reg, offset: i32, src: Reg) {
        self.lock_prefix();
        self.rex(false, src as u8, 0, base as u8);
        self.emit_u8(0x0F); self.emit_u8(0xB0);
        self.emit_modrm_mem(src, base, offset);
    }
    ...
    pub fn pextrw_reg_xmm_imm8(&mut self, dst: Reg, src: XmmReg, imm: u8) {
        self.emit_u8(0x66); self.rex(true, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0xC5);
        self.modrm(3, dst as u8, src as u8);
        self.emit_u8(imm);
    }
    ...
    pub fn pinsrb_xmm_reg_imm8(&mut self, dst: XmmReg, src: Reg, imm: u8) {
        self.emit_u8(0x66); self.rex(true, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0x3A); self.emit_u8(0x20);
        self.modrm(3, dst as u8, src as u8);
        self.emit_u8(imm);
    }
    ...
    pub fn pinsrw_xmm_reg_imm8(&mut self, dst: XmmReg, src: Reg, imm: u8) {
        self.emit_u8(0x66); self.rex(true, dst as u8, 0, src as u8);
        self.emit_u8(0x0F); self.emit_u8(0xC4);
        self.modrm(3, dst as u8, src as u8);
        self.emit_u8(imm);
    }
