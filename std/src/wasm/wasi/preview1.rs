use crate::fs;
use crate::rust_alloc::{collections::BTreeMap, format, string::String, string::ToString, vec, vec::Vec};
...
use crate::debugln;
pub(crate) struct WasiFile {
    pub(crate) file: fs::File,
    pub(crate) _path: String,
}
pub(crate) static FD_TABLE: Mutex<BTreeMap<i32, WasiFile>> = Mutex::new(BTreeMap::new());
pub(crate) static NEXT_FD: Mutex<i32> = Mutex::new(10);
pub(crate) static mut RANDOM_STATE: u64 = 0;
...
fn args_sizes_get<T: Config>(store: &mut Store<'_, T>, args: Vec<Value>) -> Result<Vec<Value>, HaltExecutionError> {
    let c_ptr = match args.get(0) {
        Some(Value::I32(v)) => *v as u32,
        _ => return Ok(vec![Value::I32(28)])
    };
    let b_ptr = match args.get(1) {
        Some(Value::I32(v)) => *v as u32,
        _ => return Ok(vec![Value::I32(28)])
    };
    let args_vec: Vec<String> = crate::env::args().collect();
...
fn fd_read<T: Config>(store: &mut Store<'_, T>, args: Vec<Value>) -> Result<Vec<Value>, HaltExecutionError> {
    ...
    let i_ptr = match args.get(1) {
        Some(Value::I32(v)) => *v as u32,
        _ => return Ok(vec![Value::I32(28)])
    };
    let i_len = match args.get(2) {
        Some(Value::I32(v)) => *v as u32,
        _ => return Ok(vec![Value::I32(28)])
    };
    let n_ptr = match args.get(3) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    let mut total = 0;
...
fn fd_tell<T: Config>(store: &mut Store<'_, T>, args: Vec<Value>) -> Result<Vec<Value>, HaltExecutionError> {
    ...
    let p_ptr = match args.get(1) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    if let Some(wf) = FD_TABLE.lock().get_mut(&fd) {
        use crate::io::{Seek, SeekFrom};
        match wf.file.seek(SeekFrom::Current(0)) {
            Ok(n) => {
...
fn fd_write<T: Config>(store: &mut Store<'_, T>, args: Vec<Value>) -> Result<Vec<Value>, HaltExecutionError> {
    ...
    for i in 0..i_len {
        ...
        if fd >= 1 && fd <= 2 {
            if let Ok(s) = core::str::from_utf8(&b) { crate::debug_print!("{}", s); } else { crate::debug_print!("{:?}", b); }
            total += b_len;
        } else {
            if let Some(wf) = FD_TABLE.lock().get_mut(&fd) {
                use crate::io::Write;
                if let Ok(n) = wf.file.write(&b) { total += n as u32; }
            }
        }
    }
...
fn path_open<T: Config>(store: &mut Store<'_, T>, args: Vec<Value>) -> Result<Vec<Value>, HaltExecutionError> {
    let dirfd = match args.get(0) {
        Some(Value::I32(v)) => *v as i32,
        _ => -1
    };
    let ptr = match args.get(2) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    ...
    let of = match args.get(4) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    let f_ptr = match args.get(8) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    if dirfd != 3 { return Ok(vec![Value::I32(76)]); }
    ...
    if ps.contains("..") { return Ok(vec![Value::I32(76)]); }
...
fn random_get<T: Config>(store: &mut Store<'_, T>, args: Vec<Value>) -> Result<Vec<Value>, HaltExecutionError> {
    ...
    unsafe {
        if RANDOM_STATE == 0 { RANDOM_STATE = crate::os::get_system_ticks().wrapping_add(0xACE1BADE); }
        let mut b = vec![0u8; len as usize];
        for i in 0..len as usize {
            RANDOM_STATE ^= RANDOM_STATE << 13;
            RANDOM_STATE ^= RANDOM_STATE >> 17;
            RANDOM_STATE ^= RANDOM_STATE << 5;
            b[i] = (RANDOM_STATE & 0xFF) as u8;
        }
        if write_bytes(store, ptr, &b).is_err() { return Ok(vec![Value::I32(28)]); }
    }
    Ok(vec![Value::I32(0)])
...
fn poll_oneoff<T: Config>(store: &mut Store<'_, T>, args: Vec<Value>) -> Result<Vec<Value>, HaltExecutionError> {
    let in_p = match args.get(0) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    let out_p = match args.get(1) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    let nsub = match args.get(2) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    let nev_p = match args.get(3) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    let mut nev = 0;
    for i in 0..nsub {
        ...
        if sub[8] == 0 {
            ...
            if u16::from_le_bytes(sub[40..42].try_into().unwrap()) == 0 {
                let ms = to / 1_000_000;
                let chunks = ms / 10;
                for _ in 0..chunks {
                    crate::os::sleep(10u64);
                    crate::os::yield_task();
                }
                crate::os::sleep(ms % 10);
            }
            let eb = out_p + (nev * 32);
            ...
            let eb = out_p + (nev * 32);
            let _ = write_u64(store, eb, ud);
            let _ = write_u16(store, eb + 8, 58);
            nev += 1;
        }
    }
...
fn path_create_directory<T: Config>(store: &mut Store<'_, T>, args: Vec<Value>) -> Result<Vec<Value>, HaltExecutionError> {
    ...
    match fs::create_dir(&format!("@0xE0/{}", cp)) {
        Ok(_) => Ok(vec![Value::I32(0)]),
        Err(_) => Ok(vec![Value::I32(28)])
    }
}
fn path_remove_directory<T: Config>(store: &mut Store<'_, T>, args: Vec<Value>) -> Result<Vec<Value>, HaltExecutionError> {
    ...
    if read_bytes(store, ptr, &mut pb).is_err() { return Ok(vec![Value::I32(21)]); }
    let cp = String::from_utf8_lossy(&pb).into_owned().trim_start_matches('.').trim_start_matches('/').to_string();
    match fs::remove_dir(&format!("@0xE0/{}", cp)) {
        Ok(_) => Ok(vec![Value::I32(0)]),
        Err(_) => Ok(vec![Value::I32(28)])
    }
}
fn path_unlink_file<T: Config>(store: &mut Store<'_, T>, args: Vec<Value>) -> Result<Vec<Value>, HaltExecutionError> {
    ...
    if read_bytes(store, ptr, &mut pb).is_err() { return Ok(vec![Value::I32(21)]); }
    let cp = String::from_utf8_lossy(&pb).into_owned().trim_start_matches('.').trim_start_matches('/').to_string();
    match fs::remove_file(&format!("@0xE0/{}", cp)) {
        Ok(_) => Ok(vec![Value::I32(0)]),
        Err(_) => Ok(vec![Value::I32(28)])
    }
}
fn path_rename<T: Config>(store: &mut Store<'_, T>, args: Vec<Value>) -> Result<Vec<Value>, HaltExecutionError> {
    let o_ptr = match args.get(1) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    let o_len = match args.get(2) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    let n_ptr = match args.get(4) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    let n_len = match args.get(5) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    let mut ob = vec![0u8; o_len as usize];
    let mut nb = vec![0u8; n_len as usize];
    ...
    match fs::rename(&format!("@0xE0/{}", co), &format!("@0xE0/{}", cn)) {
        Ok(_) => Ok(vec![Value::I32(0)]),
        Err(_) => Ok(vec![Value::I32(28)])
    }
}
...
fn fd_readdir<T: Config>(store: &mut Store<'_, T>, args: Vec<Value>) -> Result<Vec<Value>, HaltExecutionError> {
    ...
    let b_len = match args.get(2) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
    let ck = match args.get(3) {
        Some(Value::I64(v)) => *v as u64,
        _ => 0
    };
    let u_ptr = match args.get(4) {
        Some(Value::I32(v)) => *v as u32,
        _ => 0
    };
