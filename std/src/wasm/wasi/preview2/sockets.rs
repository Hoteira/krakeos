use super::{call_cabi_realloc, read_mem, write_bytes, write_u32, write_u64};
...
pub fn adapter_close_badfd<T: Config>(
    _: &mut Store<'_, T>,
    _: Vec<Value>,
...
pub fn tcp_create_socket<T: Config>(
    store: &mut Store<'_, T>,
    args: Vec<Value>,
) -> Result<Vec<Value>, HaltExecutionError> {
    let address_family = match args.get(0) {
        Some(Value::I32(v)) => *v,
        _ => return Err(HaltExecutionError(1)),
    };
    let result_ptr = match args.get(1) {
    ...
    let mut buf = [0u8; 8];
    if res == u64::MAX {
        crate::debugln!("WASI tcp_create_socket: sys_socket returned u64::MAX!");
        buf[0] = 1;
    } else {
        crate::debugln!(
            "WASI tcp_create_socket: sys_socket returned socket fd {}",
            res
        );
        buf[0] = 0;
...
pub fn tcp_start_bind<T: Config>(
    store: &mut Store<'_, T>,
    args: Vec<Value>,
) -> Result<Vec<Value>, HaltExecutionError> {
    let socket = match args.get(0) {
        Some(Value::I32(v)) => *v,
        _ => return Err(HaltExecutionError(1)),
    };
    let ip_addr_ptr = match args.get(2) {
        Some(Value::I32(v)) => *v as u32,
        _ => return Err(HaltExecutionError(1)),
    };
    let result_ptr = match args.get(3) {
        Some(Value::I32(v)) => *v as u32,
        _ => return Err(HaltExecutionError(1)),
    };
    ...
    let mem_addr = *store
        ...
        .mem_addrs
        .get(0)
        .ok_or(HaltExecutionError(1))?;
    let ip_addr = store.memories.get(mem_addr).mem.get_base_ptr() as u64 + ip_addr_ptr as u64;

    #[cfg(not(target_arch = "wasm32"))]
    let res = unsafe { crate::sys::syscall6(49, socket as u64, ip_addr, 16, 0, 0, 0) };
    #[cfg(target_arch = "wasm32")]
    let res = u64::MAX;
...
pub fn tcp_finish_bind<T: Config>(
    store: &mut Store<'_, T>,
    args: Vec<Value>,
) -> Result<Vec<Value>, HaltExecutionError> {
    let result_ptr = match args.get(1) {
    ...
    let mut buf = [0u8; 4];
    buf[0] = 0; // synchronous on KrakeOS
    write_bytes(store, result_ptr, &buf).map_err(|_| HaltExecutionError(1))?;
    Ok(vec![])
}

pub fn tcp_start_connect<T: Config>(
    store: &mut Store<'_, T>,
    args: Vec<Value>,
) -> Result<Vec<Value>, HaltExecutionError> {
    let socket = match args.get(0) {
    ...
    crate::debugln!(
        "WASI tcp_start_connect: socket={} ip_addr_ptr={} mapped_host_ptr={:#010X}",
        socket,
        ip_addr_ptr,
        ip_addr
    );
    ...
    #[cfg(not(target_arch = "wasm32"))]
    let res = unsafe { crate::sys::syscall6(42, socket as u64, ip_addr, 16, 0, 0, 0) };
    #[cfg(target_arch = "wasm32")]
    let res = u64::MAX;
...
pub fn tcp_finish_connect<T: Config>(
    store: &mut Store<'_, T>,
    args: Vec<Value>,
) -> Result<Vec<Value>, HaltExecutionError> {
    let result_ptr = match args.get(1) {
    ...
    let mut buf = [0u8; 4];
    buf[0] = 0; // blocking on KrakeOS
    write_bytes(store, result_ptr, &buf).map_err(|_| HaltExecutionError(1))?;
    Ok(vec![])
}

pub fn tcp_start_listen<T: Config>(
    store: &mut Store<'_, T>,
    args: Vec<Value>,
) -> Result<Vec<Value>, HaltExecutionError> {
    let socket = match args.get(0) {
...
pub fn tcp_finish_listen<T: Config>(
    store: &mut Store<'_, T>,
    args: Vec<Value>,
) -> Result<Vec<Value>, HaltExecutionError> {
    let result_ptr = match args.get(1) {
...
pub fn tcp_send<T: Config>(
    ...
    let mem_addr = *store
        ...
        .get(0)
        .ok_or(HaltExecutionError(1))?;
    let host_ptr = store.memories.get(mem_addr).mem.get_base_ptr() as u64 + buf_ptr as u64;

    #[cfg(not(target_arch = "wasm32"))]
    let res = unsafe { crate::sys::syscall6(52, socket as u64, host_ptr, buf_len as u64, 0, 0, 0) };
    #[cfg(target_arch = "wasm32")]
    let res = u64::MAX;
...
pub fn tcp_recv<T: Config>(
    ...
    #[cfg(not(target_arch = "wasm32"))]
    let res = unsafe { crate::sys::syscall6(53, socket as u64, host_ptr, buf_len as u64, 0, 0, 0) };
    #[cfg(target_arch = "wasm32")]
    let res = u64::MAX;
...
pub fn instance_network<T: Config>(
    _: &mut Store<'_, T>,
    _: Vec<Value>,
) -> Result<Vec<Value>, HaltExecutionError> {
    Ok(vec![Value::I32(0)])
...
pub fn resolve_addresses<T: Config>(
    store: &mut Store<'_, T>,
    args: Vec<Value>,
) -> Result<Vec<Value>, HaltExecutionError> {
    let result_ptr = match args.get(3) {
...
pub fn create_udp_socket<T: Config>(
    store: &mut Store<'_, T>,
    args: Vec<Value>,
) -> Result<Vec<Value>, HaltExecutionError> {
    let address_family = match args.get(0) {
...
pub fn start_bind<T: Config>(
    ...
    #[cfg(not(target_arch = "wasm32"))]
    let res =
        unsafe { crate::sys::syscall6(49, socket as u64, ip_addr.as_ptr() as u64, 16, 0, 0, 0) };
    #[cfg(target_arch = "wasm32")]
    let res = u64::MAX;
...
pub fn send<T: Config>(
    ...
    let dest_addr_ptr = match args.get(3) {
        Some(Value::I32(v)) => *v as u32,
        _ => return Err(HaltExecutionError(1)),
    };
    let result_ptr = match args.get(4) {
        Some(Value::I32(v)) => *v as u32,
        _ => return Err(HaltExecutionError(1)),
    };
...
pub fn receive<T: Config>(
    ...
    let max_results = match args.get(1) {
        Some(Value::I64(v)) => *v,
        _ => return Err(HaltExecutionError(1)),
    };
    let result_ptr = match args.get(2) {
        Some(Value::I32(v)) => *v as u32,
        _ => return Err(HaltExecutionError(1)),
    };
    ...
    write_bytes(
        store,
        result_ptr,
        &header[..32 + if res != u64::MAX { res as usize } else { 0 }],
    )
