...
use crate::wasm::core::{indices::MemIdx, reader::span::Span};
...
impl Debug for DataSegment {
    fn fmt(&self, f: &mut Formatter<'_>) -> core::fmt::Result {
        let mut init_str = crate::rust_alloc::string::String::new();

        let iter = self.init.iter().peekable();
        // only if it's valid do we print is as a normal utf-8 char, otherwise, hex
        for &byte in iter {
            if let Ok(valid_char) = crate::rust_alloc::string::String::from_utf8(Vec::from(&[byte])) {
