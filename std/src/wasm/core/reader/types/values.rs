...
impl WasmReader<'_> {
    ...
    /// Reads a name from a component. 
    ///
    /// According to the Component Model binary format, a component name (import/export name)
    /// is preceded by a tag byte: 0x00 for a plain name, 0x01 for an interface name,
    /// 0x02 for a versioned interface name.
    pub fn read_component_name(&mut self) -> Result<crate::rust_alloc::string::String, ValidationError> {
        ...
        match tag {
            ...
            _ => {
                ...
                Err(ValidationError::Component(crate::wasm::component::error::ComponentError::MalformedSectionId(tag)))
            }
        }
    }
