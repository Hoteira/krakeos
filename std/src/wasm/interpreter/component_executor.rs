...
use crate::wasm::common::interop::Linker;
...
use crate::wasm::interpreter::store::ExternVal;
            ...
            ComponentItem::CoreInstance(core_inst_def) => {
                match &core_inst_def.kind {
                    crate::wasm::component::types::CoreInstanceKind::Instantiate {
                        ...
                        let validation_info = match crate::wasm::common::validation::validate(module_bytes)
                        {
                            Ok(info) => info,
                            Err(e) => {
                        ...
                        for import in &validation_info.imports {
                            ...
                            if !resolved {
                                ...
                                if import.module_name == "env" && import.name == "memory" {
                                    let mem_ty = crate::wasm::common::reader::types::MemType {
                                        limits: crate::wasm::common::reader::types::Limits {
                                            min: 256, // 16 MiB
                                            max: None,
                                        },
                                    };
                        ...
                        if let Ok(outcome) = store.module_instantiate_unchecked(
                            ...
                            if let Some(import_setup_func) = module_inst.exports.get("$imports") {
                                if let Some(func_addr) = import_setup_func.as_func() {
                                    let params: Vec<crate::wasm::common::value::Value> = extern_vals
                                        .iter()
                                        .filter_map(|ev| {
                                            if let crate::wasm::interpreter::store::ExternVal::Func(addr) = ev {
                                                Some(crate::wasm::common::value::Value::Ref(
                                                    crate::wasm::common::value::Ref::Func(*addr),
                                                ))
                                            } else {
                                                None
                                            }
