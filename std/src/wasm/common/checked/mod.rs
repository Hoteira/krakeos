...
use crate::wasm::interpreter::store::{ExternVal, Store, StoreId, InstantiationOutcome};
use crate::wasm::interpreter::store::addrs::{FuncAddr, GlobalAddr, MemAddr, ModuleAddr, TableAddr};
use crate::wasm::interpreter::resumable::{ResumableRef, RunState};
...
impl<'b, T: crate::wasm::common::config::Config> Store<'b, T> {
...
impl<T> AbstractStored for Stored<T> {
    type BareTy = T;
    unsafe fn from_bare(inner: T, id: StoreId) -> Self {
        Self { inner, id }
    }
    fn id(&self) -> Option<StoreId> {
        Some(self.id)
...
impl<T> Stored<T> {
    pub fn as_mut(&mut self) -> Stored<&mut T> {
        Stored {
            id: self.id,
...
impl StoredExternVal {
    pub fn as_func(self) -> Option<Stored<FuncAddr>> {
        if let StoredExternVal::Func(f) = self {
            Some(f)
        } else {
            None
        }
    ...
    fn into_bare(self) -> RunState {
        match self {
            Self::Finished {
                ...
                values: values.into_iter().map(|v| v.into_bare()).collect(),
                maybe_remaining_fuel,
            },
            Self::Resumable {
                resumable_ref,
                ...
                resumable_ref: resumable_ref.into_bare(),
                required_fuel,
            },
        }
    }
...
impl AbstractStored for StoredInstantiationOutcome {
    ...
    unsafe fn from_bare(bare: InstantiationOutcome, id: StoreId) -> Self {
        Self {
            module_addr: Stored::from_bare(bare.module_addr, id),
            maybe_remaining_fuel: bare.maybe_remaining_fuel,
        }
    }
    fn id(&self) -> Option<StoreId> {
        self.module_addr.id()
    }
    fn into_bare(self) -> InstantiationOutcome {
...
fn try_unwrap_values<S: AbstractStored>(
    ...
    stored
        .into_iter()
        .map(|s| s.try_unwrap_into_bare(id))
        .collect()
}
