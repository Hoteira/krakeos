...
use crate::wasm::{RefType, RuntimeError, ValidationInfo};
use instances::{
...
impl<'b, T: Config> Store<'b, T> {
    ...
    pub fn module_instantiate_unchecked(
        ...
        for (i, DataSegment { init, mode }) in validation_info.data.iter().enumerate() {
            match mode {
                crate::wasm::core::reader::types::data::DataMode::Active(DataModeActive {
                                                                             memory_idx,
                                                                             offset: dinstr_i,
                                                                         }) => {
                ...
                crate::wasm::core::reader::types::data::DataMode::Passive => (),
            }
        }
    ...
    pub fn func_alloc_unchecked(
        &mut self,
        func_type: FuncType,
        host_func: for<'a> fn(&mut Store<'a, T>, Vec<Value>) -> Result<Vec<Value>, HaltExecutionError>,
    ) -> FuncAddr {
            ...
            ResumableRef::Fresh(FreshResumableRef {
                ...
                match func_inst {
                    FuncInst::HostFunc(host_func_inst) => {
                        ...
                        let returns = hostcode(self, params);
                        ...
                        // Re-borrow func_inst to check return types
                        // But we can't because self is borrowed mutably by hostcode? 
                        // No, hostcode returned. self is free.
                        // But func_inst reference is invalid because self was mutated.
                        // We need to fetch it again or capture types before call.

                        let func_inst = self.functions.get(func_addr);
                        let FuncInst::HostFunc(host_func_inst) = func_inst else { unreachable!() };
...
impl StoreId {
    ...
    )] // reason = "StoreId::default() might be misunderstood to be some
