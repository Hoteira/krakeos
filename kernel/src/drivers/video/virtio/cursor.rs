use crate::debugln;

use super::consts::*;
use super::queue::send_command_queue;
use super::structs::*;

pub const CURSOR_ID: u32 = 3;

static mut MOVE_REQ: VirtioGpuUpdateCursor = unsafe { core::mem::zeroed() };

pub fn setup_cursor(phys_ptr: u64, width: u32, height: u32, x: u32, y: u32) {
    let cursor_id = CURSOR_ID;

    let req_create = VirtioGpuResourceCreate2d {
        hdr: VirtioGpuCtrlHeader {
            type_: VIRTIO_GPU_CMD_RESOURCE_CREATE_2D,
            flags: 0,
            fence_id: 0,
            ctx_id: 0,
            ring_idx: 0,
            padding: [0; 3],
        },
        resource_id: cursor_id,
        format: VIRTIO_GPU_FORMAT_B8G8R8A8_UNORM,
        width,
        height,
    };
    let resp_create: VirtioGpuCtrlHeader = unsafe { core::mem::zeroed() };

    let req_create_phys = crate::memory::paging::virt_to_phys(&req_create as *const _ as u64);
    let resp_create_phys = crate::memory::paging::virt_to_phys(&resp_create as *const _ as u64);

    send_command_queue(
        0,
        &[req_create_phys],
        &[core::mem::size_of_val(&req_create) as u32],
        &[resp_create_phys],
        &[core::mem::size_of_val(&resp_create) as u32],
        true,
    );


    #[repr(C)]
    struct AttachRequest {
        hdr: VirtioGpuResourceAttachBacking,
        entry: VirtioGpuMemEntry,
    }

    let req_attach = AttachRequest {
        hdr: VirtioGpuResourceAttachBacking {
            hdr: VirtioGpuCtrlHeader {
                type_: VIRTIO_GPU_CMD_RESOURCE_ATTACH_BACKING,
                flags: 0,
                fence_id: 0,
                ctx_id: 0,
                ring_idx: 0,
                padding: [0; 3],
            },
            resource_id: cursor_id,
            nr_entries: 1,
        },
        entry: VirtioGpuMemEntry {
            addr: phys_ptr,
            length: width * height * 4,
            padding: 0,
        },
    };
    let resp_attach: VirtioGpuCtrlHeader = unsafe { core::mem::zeroed() };

    let req_attach_phys = crate::memory::paging::virt_to_phys(&req_attach as *const _ as u64);
    let resp_attach_phys = crate::memory::paging::virt_to_phys(&resp_attach as *const _ as u64);

    send_command_queue(
        0,
        &[req_attach_phys],
        &[core::mem::size_of_val(&req_attach) as u32],
        &[resp_attach_phys],
        &[core::mem::size_of_val(&resp_attach) as u32],
        true,
    );


    let req_transfer = VirtioGpuTransferToHost2d {
        hdr: VirtioGpuCtrlHeader {
            type_: VIRTIO_GPU_CMD_TRANSFER_TO_HOST_2D,
            flags: 0,
            fence_id: 0,
            ctx_id: 0,
            ring_idx: 0,
            padding: [0; 3],
        },
        r: VirtioGpuRect { x: 0, y: 0, width, height },
        offset: 0,
        resource_id: cursor_id,
        padding: 0,
    };
    let resp_transfer: VirtioGpuCtrlHeader = unsafe { core::mem::zeroed() };

    let req_transfer_phys = crate::memory::paging::virt_to_phys(&req_transfer as *const _ as u64);
    let resp_transfer_phys = crate::memory::paging::virt_to_phys(&resp_transfer as *const _ as u64);

    send_command_queue(
        0,
        &[req_transfer_phys],
        &[core::mem::size_of_val(&req_transfer) as u32],
        &[resp_transfer_phys],
        &[core::mem::size_of_val(&resp_transfer) as u32],
        true,
    );


    let req_update = VirtioGpuUpdateCursor {
        hdr: VirtioGpuCtrlHeader {
            type_: VIRTIO_GPU_CMD_UPDATE_CURSOR,
            flags: 0,
            fence_id: 0,
            ctx_id: 0,
            ring_idx: 0,
            padding: [0; 3],
        },
        pos: VirtioGpuCursorPos {
            scanout_id: 0,
            x,
            y,
            padding: 0,
        },
        resource_id: cursor_id,
        hot_x: 0,
        hot_y: 0,
        padding: 0,
    };

    let req_update_phys = crate::memory::paging::virt_to_phys(&req_update as *const _ as u64);

    send_command_queue(
        1,
        &[req_update_phys],
        &[core::mem::size_of_val(&req_update) as u32],
        &[],
        &[],
        true,
    );
    debugln!("VirtIO GPU: Hardware Cursor Setup & Update Sent (Queue 1).");
}

pub fn move_cursor(x: u32, y: u32) {
    unsafe {
        MOVE_REQ = VirtioGpuUpdateCursor {
            hdr: VirtioGpuCtrlHeader {
                type_: VIRTIO_GPU_CMD_MOVE_CURSOR,
                flags: 0,
                fence_id: 0,
                ctx_id: 0,
                ring_idx: 0,
                padding: [0; 3],
            },
            pos: VirtioGpuCursorPos {
                scanout_id: 0,
                x,
                y,
                padding: 0,
            },
            resource_id: CURSOR_ID,
            hot_x: 0,
            hot_y: 0,
            padding: 0,
        };

        let req_move_phys = crate::memory::paging::virt_to_phys(&raw const MOVE_REQ as u64);

        send_command_queue(
            1,
            &[req_move_phys],
            &[core::mem::size_of::<VirtioGpuUpdateCursor>() as u32],
            &[],
            &[],
            false,
        );
    }
}
