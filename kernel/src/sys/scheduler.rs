...
pub static mut SCHEDULER: Scheduler = Scheduler {
    threads: [Thread { sp: 0, state: ThreadState::Empty, fpu_state: FpuFrame::new() }; MAX_THREADS],
    current: 0,
};
...
impl Scheduler {
    ...
    pub fn spawn_user_thread(entry: usize, arg: usize) {
        unsafe {
            ...
            let kernel_stack = alloc::alloc::alloc_zeroed(layout) as usize; if kernel_stack == 0 { crate::println!("OOM kernel stack"); loop {} }
            let user_layout = Layout::from_size_align(1024 * 1024, 4096).unwrap(); // 1MB user stack
            let user_stack = alloc::alloc::alloc_zeroed(user_layout) as usize; if user_stack == 0 { crate::println!("OOM user stack"); loop {} }
...
pub unsafe fn switch(current_sp: usize) -> usize {
    ...
    let mut next = (sched.current + 1) % MAX_THREADS;
