...
pub fn load_elf(data: &[u8], pid: u64) -> Result<u64, String> {
    ...
    for phdr in elf.program_headers() {
        if ProgramType::from(phdr.p_type) == ProgramType::Load {
            ...
            while current_page < page_end {
                ...
                vmm::map_page(current_page, PhysAddr::new(frame), flags, None);
                ...
                let segment_file_end = virt_start + phdr.p_filesz;
                ...
                if intersect_end > intersect_start {
                    ...
                    let file_offset = phdr.p_offset + (intersect_start - virt_start);
    ...
    for shdr in elf.section_headers() {
        if shdr.sh_type == 4 {
            ...
            for rela in relas {
                    ...
                    1 | 6 | 7 => {
                        if let Some(sym_tab) = dynsym_shdr {
                            ...
                            if sym_offset < data.len() {
                                ...
                                if sym.st_shndx != 0 { val = sym.st_value + load_base; }
                ...
                if found_val {
                    unsafe {
                        if let Some(phys) = vmm::get_phys(target_virt, vmm::KERNEL_PML4) {
