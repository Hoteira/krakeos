...
use crate::types::{Align, BackgroundStyle, Color, LinearGradient, Size};
use crate::window::Window;

pub type WidgetId = usize;
pub type EventHandler = fn(&mut Window, WidgetId);

/// Decode a PNG and nearest-neighbor scale it to (w, h) as 0xAARRGGBB.
fn load_image(data: &[u8], w: usize, h: usize) -> Result<Vec<u32>, String> {
    ...
    if iw == 0 || ih == 0 || w == 0 || h == 0 {
        return Err(String::from("zero-sized image"));
    }
    ...
    for y in 0..h {
        ...
        for x in 0..w {
            ...
            let a = if bpp == 4 { buf[i + 3] as u32 } else { 255 };
...
impl Widget {
    ...
    pub fn raw_image(id: WidgetId, data: &[u8], width: usize, height: usize) -> Self {
        let mut rasterized_buffer = vec![0u32; width * height];
    ...
    pub fn handle_scroll(&mut self, delta: i8) {
        ...
        if delta > 0 {
            geo.scroll_offset_y = geo.scroll_offset_y.saturating_add((delta.unsigned_abs() as usize) * scroll_step);
        } else {
            geo.scroll_offset_y = geo.scroll_offset_y.saturating_sub((delta.unsigned_abs() as usize) * scroll_step);
        }
    ...
    pub fn get_text(&self) -> String {
        match self {
            ...
            Widget::Label { text, .. } => text.text.clone(),
            _ => String::new(),
        }
    }
    ...
    pub fn handle_key(&mut self, key: char) {
        if let Widget::TextInput { text, geometry, .. } = self {
            ...
            } else if text.max_len.map_or(true, |m| text.text.len() < m) {
                text.text.push(key);
            }
    ...
    pub fn update_layout(
        ...
        match self {
            ...
            Widget::Image { geometry, source_data, rasterized_buffer, last_raster_size } => {
                if geometry.width > 0
                    && geometry.height > 0
                    && (geometry.width != last_raster_size.0 || geometry.height != last_raster_size.1)
                    && !source_data.is_empty()
                {
                    if let Ok(buffer) = load_image(source_data, geometry.width, geometry.height) {
            ...
            Widget::Image { geometry, rasterized_buffer, last_raster_size, .. } => {
                if !rasterized_buffer.is_empty() {
                    ...
                    for row in 0..copy_h {
                        ...
                        if actual_copy_w > 0
                            && dest_start + actual_copy_w <= fb_len
                            && src_start + actual_copy_w <= rasterized_buffer.len()
                        {
                            framebuffer[dest_start..dest_start + actual_copy_w]
                                .copy_from_slice(&rasterized_buffer[src_start..src_start + actual_copy_w]);
                        }
                    }
                }
            }
